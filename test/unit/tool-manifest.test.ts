...
test("manifest counts per group match TOOL_GROUP_COUNTS (evaluate and captcha_solve excluded)", () => {
  const counts: Record<string, number> = {};
  for (const t of TOOL_MANIFEST) {
    if (t.name !== "evaluate" && t.name !== "captcha_solve") counts[t.group] = (counts[t.group] ?? 0) + 1;
  }
