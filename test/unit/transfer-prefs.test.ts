...
test("tool counts: legacy defaults = 32 + read 9 = 41; everything + evaluate = 74", () => {
  ...
  assert.equal(enabledToolCount(all, false), 73);
  assert.equal(enabledToolCount(all, true), 74);
  assert.equal(TOOL_GROUP_COUNTS.toolsInteract, 11);
  assert.equal(TOOL_GROUP_COUNTS.toolsState, 12);
});
