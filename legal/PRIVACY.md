...
- If you turn on **CAPTCHA solver** in Settings → System, what leaves this computer depends on the provider you pick. **CapSolver** sends the page URL and the CAPTCHA's public site key (never page content or your answers) to CapSolver’s API to get a token, which the App injects back into the page; with **Auto-solve when detected** on, this happens automatically as soon as a challenge is seen. **Connected assistant** sends pictures (and, for audio challenges, the audio) of visible CAPTCHA widgets to the connected MCP client. **OpenAI / Gemini** send those pictures/audio to that provider using the API key you saved. All keys stay in the App’s local data folder (`captcha-solver.json`) and are not shown to connected assistants. Turn the solver off, turn off auto-solve, or remove the key, to stop those requests.
