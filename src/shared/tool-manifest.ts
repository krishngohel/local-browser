...
export const TOOL_MANIFEST: ToolManifestEntry[] = [
  ...
  { name: "captcha_solve", group: "toolsRead", description: "Attempt to solve a visible CAPTCHA. Settings → System chooses Connected assistant (this model looks at challenge images Echo returns, then call captcha_solve again with tiles, text, or offsetPx) or OpenAI/Gemini with an API key. Supports text CAPTCHAs, reCAPTCHA v2 image grids, and slider puzzles. Score-based invisible checks cannot be solved. Requires the user to enable the solver. Optionally target a specific tabId (see tabs_list)." },
